...
(function () {
    ...
    // ─────────────────────────────────────────────────────────
    // TIMELINE LAYOUT
    // Strip-based structure: songs strip, divider strip, board strip, notes strip.
    // Every turn occupies one "column index" in every strip — alignment is automatic.
    // Each cell carries data-turn-id so we can group operations across strips.
    // ─────────────────────────────────────────────────────────
    let turnCount = 0;

    function setupTimelineLayout() {
        const lanes = $('#lanes');
        lanes.classList.add('timeline');
        lanes.innerHTML = '';

        const make = (cls, id) => el('div', { class: `strip ${cls}`, id });
        const songsStrip = make('songs-strip', 'songs-strip');
        const dividerStrip = make('divider-strip', 'divider-strip');
        const boardStrip = make('board-strip', 'board-strip');
        const notesStrip = make('notes-strip', 'notes-strip');

        // strip labels (small vertical tag on the left of each strip)
        songsStrip.appendChild(el('span', { class: 'strip-label' }, ['Songs ●']));
        boardStrip.appendChild(el('span', { class: 'strip-label' }, ['Board ●']));
        notesStrip.appendChild(el('span', { class: 'strip-label' }, ['Notes']));

        lanes.appendChild(songsStrip);
        lanes.appendChild(dividerStrip);
        lanes.appendChild(boardStrip);
        lanes.appendChild(notesStrip);

        // add-turn handle on the right side
        const addBtn = el('button', { class: 'add-turn-vertical', id: 'add-turn-vertical', title: 'Add a turn' }, ['+   add turn']);
        addBtn.addEventListener('click', () => addTurnColumn());
        lanes.appendChild(addBtn);
    }

    function makeTurnCells(turnNum, turnId) {
        // SONG cell
        const songCell = el('div', { class: 'song-cell', dataset: { turnId } });
        const songGrid = el('div', { class: 'lane-grid', dataset: { gridType: 'song' } });
        songGrid.appendChild(makeLaneRow('song', turnId, 1));
        attachLaneGridDrop(songGrid);
        songCell.appendChild(songGrid);

        // DIVIDER cell with the turn chip
        const dividerCell = el('div', { class: 'divider-cell', dataset: { turnId } });
        const chip = el('div', { class: 'turn-chip' });
        chip.appendChild(el('span', { class: 'turn-pip' }, ['Turn']));
        const tnum = el('span', { class: 'turn-num', contenteditable: 'true' }, [String(turnNum)]);
        tnum.addEventListener('input', saveSession);
        tnum.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); tnum.blur(); } });
        chip.appendChild(tnum);
        const inkPill = el('span', { class: 'ink-pill' });
        inkPill.appendChild(el('span', { class: 'dot' }));
        const inkNum = el('span', { class: 'ink-num', contenteditable: 'true' }, [String(turnNum)]);
        inkNum.addEventListener('input', saveSession);
        inkNum.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); inkNum.blur(); } });
        inkPill.appendChild(inkNum);
        inkPill.appendChild(el('span', { class: 'mono dim' }, [' ink']));
        chip.appendChild(inkPill);
        const removeBtn = el('button', { class: 'remove-turn', title: 'Remove this turn' });
        ...
        removeBtn.addEventListener('click', () => removeTurn(turnId));
        chip.appendChild(removeBtn);
        dividerCell.appendChild(chip);

        // BOARD cell
        const boardCell = el('div', { class: 'board-cell', dataset: { turnId } });
        const boardGrid = el('div', { class: 'lane-grid', dataset: { gridType: 'board' } });
        boardGrid.appendChild(makeLaneRow('board', turnId, 1));
        attachLaneGridDrop(boardGrid);
        boardCell.appendChild(boardGrid);

        // NOTES cell
        const notesCell = el('div', { class: 'notes-cell', dataset: { turnId } });
        const ta = el('textarea', { class: 'turn-comment', placeholder: `Turn ${turnNum} notes…`, rows: 1 });
        autosize(ta);
        ta.addEventListener('input', saveSession);
        notesCell.appendChild(ta);

        return { songCell, dividerCell, boardCell, notesCell };
    }

    function addTurnColumn(turnNum) {
        turnCount++;
        const turnId = turnCount;
        const next = turnNum ?? ($('#divider-strip').querySelectorAll('.divider-cell').length + 1);
        const { songCell, dividerCell, boardCell, notesCell } = makeTurnCells(next, turnId);
        $('#songs-strip').appendChild(songCell);
        $('#divider-strip').appendChild(dividerCell);
        $('#board-strip').appendChild(boardCell);
        $('#notes-strip').appendChild(notesCell);
        renderArrows();
        saveSession();
        return dividerCell;
    }

    function getTurnCells(turnId) {
        return {
            songCell: document.querySelector(`#songs-strip .song-cell[data-turn-id="${turnId}"]`),
            dividerCell: document.querySelector(`#divider-strip .divider-cell[data-turn-id="${turnId}"]`),
            boardCell: document.querySelector(`#board-strip .board-cell[data-turn-id="${turnId}"]`),
            notesCell: document.querySelector(`#notes-strip .notes-cell[data-turn-id="${turnId}"]`)
        };
    }

    function removeTurn(turnId) {
        const cells = getTurnCells(turnId);
        const cardIds = new Set();
        Object.values(cells).forEach(c => {
            if (!c) return;
            c.querySelectorAll('.placed-card').forEach(p => cardIds.add(p.id));
            c.remove();
        });
        state.arrows = state.arrows.filter(a => !cardIds.has(a.startId) && !cardIds.has(a.endId));
        renderArrows();
        saveSession();
    }

    function attachAddTurnHandle() { /* no-op in timeline mode — the handle is permanent */ }

    function findColumnByTurnNumber(num) {
        const divCell = Array.from(document.querySelectorAll('#divider-strip .divider-cell'))
            .find(c => parseInt(c.querySelector('.turn-num').textContent, 10) === num);
        if (!divCell) return null;
        const turnId = divCell.dataset.turnId;
        // Synthesise a column-like object for compatibility with existing code paths.
        return {
            dataset: { turnId },
            querySelector: (sel) => {
                if (sel === '.turn-num') return divCell.querySelector('.turn-num');
                if (sel === '.ink-num') return divCell.querySelector('.ink-num');
                const cells = getTurnCells(turnId);
                if (sel.includes("data-grid-type=\"song\"") || sel.includes("data-grid-type='song'") || sel.includes('[data-grid-type="song"]')) return cells.songCell?.querySelector('.lane-grid');
                if (sel.includes('board')) return cells.boardCell?.querySelector('.lane-grid');
                return null;
            }
        };
    }
    ...
    function clearBoard(fullReset = true) {
        ['#songs-strip', '#divider-strip', '#board-strip', '#notes-strip'].forEach(sel => {
            const strip = $(sel);
            if (!strip) return;
            // remove all cells, keep the strip-label
            Array.from(strip.children).forEach(child => {
                if (!child.classList.contains('strip-label')) child.remove();
            });
        });
        state.arrows = [];
        renderArrows();
    ...
    function setupRowDropZone(row) {
        ...
        row.addEventListener('drop', (e) => {
            ...
            if (grid?.classList.contains('lane-grid')) {
                const turnHost = grid.closest('[data-turn-id]');
                const turnId = turnHost?.dataset.turnId;
    ...
    function removePlacedCard(node) {
        ...
        if (row?.classList.contains('lane-row')) {
            ...
            const turnHost = grid.closest('[data-turn-id]');
            rebuildOrSeparators(grid, grid.dataset.gridType, turnHost?.dataset.turnId);
        }
        renderArrows();
        saveSession();
    }
    ...
    function ensureTurns(n) {
        const existing = $$('#divider-strip .divider-cell').length;
    ...
    function autoPopulate() {
        ...
        $$('#lanes .lane-grid').forEach(g => {
            const host = g.closest('[data-turn-id]');
            rebuildOrSeparators(g, g.dataset.gridType, host?.dataset.turnId);
        });
        renderArrows();
        saveSession();
    }
    ...
    function gatherState() {
        ...
        const turnData = [];
        const turnComments = [];
        $$('#divider-strip .divider-cell').forEach(divCell => {
            const turnId = divCell.dataset.turnId;
            const turn = divCell.querySelector('.turn-num').textContent.trim();
            const ink = divCell.querySelector('.ink-num').textContent.trim();
            const songCell = document.querySelector(`#songs-strip .song-cell[data-turn-id="${turnId}"]`);
            const boardCell = document.querySelector(`#board-strip .board-cell[data-turn-id="${turnId}"]`);
            const notesCell = document.querySelector(`#notes-strip .notes-cell[data-turn-id="${turnId}"]`);
            const songRows = songCell ? songCell.querySelectorAll('.lane-row').length : 0;
            const boardRows = boardCell ? boardCell.querySelectorAll('.lane-row').length : 0;
            turnData.push({ id: turnId, turn, ink, songRowCount: songRows, boardRowCount: boardRows });
            turnComments.push(notesCell?.querySelector('.turn-comment')?.value || '');
        });
    ...
    function loadState(s, fullClear = true) {
        ...
        (s.turnData || []).forEach(td => {
            const divCell = addTurnColumn();
            const turnId = divCell.dataset.turnId;
            divCell.querySelector('.turn-num').textContent = td.turn;
            divCell.querySelector('.ink-num').textContent = td.ink;
            const songCell = document.querySelector(`#songs-strip .song-cell[data-turn-id="${turnId}"]`);
            const boardCell = document.querySelector(`#board-strip .board-cell[data-turn-id="${turnId}"]`);
            const songGrid = songCell?.querySelector('.lane-grid');
            const boardGrid = boardCell?.querySelector('.lane-grid');
            if (songGrid) {
                songGrid.innerHTML = '';
        ...
        (s.turnComments || []).forEach((tc, idx) => {
            const notesCell = $$('#notes-strip .notes-cell')[idx];
            if (notesCell) {
                const ta = notesCell.querySelector('.turn-comment');
                if (ta) { ta.value = tc; ta.dispatchEvent(new Event('input')); }
            }
        });
    ...
    // ─────────────────────────────────────────────────────────
    const SESSION_KEY = 'turnMapperSession_timeline_v1';
    ...
    function generateHumanReadableText() {
        ...
        const cols = $$('#divider-strip .divider-cell');
        cols.forEach(col => {
        ...
        data.arrows.forEach(arrow => {
            ...
            const sTurn = (sEl.closest('[data-turn-id]') ? document.querySelector(`#divider-strip .divider-cell[data-turn-id="${sEl.closest('[data-turn-id]').dataset.turnId}"]`)?.querySelector('.turn-num').textContent : null);
            const eTurn = (eEl.closest('[data-turn-id]') ? document.querySelector(`#divider-strip .divider-cell[data-turn-id="${eEl.closest('[data-turn-id]').dataset.turnId}"]`)?.querySelector('.turn-num').textContent : null);
        ...
        sorted.forEach(col => {
            const turnId = col.dataset.turnId;
            const turn = col.querySelector('.turn-num').textContent;
            const ink = col.querySelector('.ink-num').textContent;
            const notesCell = document.querySelector(`#notes-strip .notes-cell[data-turn-id="${turnId}"]`);
            const comment = notesCell?.querySelector('.turn-comment')?.value.trim() || '';
            const inter = interactionsByTurn[turn];
            const excluded = inter ? inter.excluded : new Set();
            const playLines = [];
            const cells = [
                document.querySelector(`#songs-strip .song-cell[data-turn-id="${turnId}"]`),
                document.querySelector(`#board-strip .board-cell[data-turn-id="${turnId}"]`)
            ].filter(Boolean);
            cells.forEach(cell => {
                $$('.lane-row', cell).forEach(row => {
